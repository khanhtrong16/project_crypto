    ...
    useEffect(() => {
        ...
        if (!chartRef.current) {
            ...
            volumeSeries.current = chartRef.current.addSeries(HistogramSeries, {
                priceScaleId: "right", // Assign the volume series to its own scale on the right
                color: "#26a69a",
                priceFormat: { type: "volume" },
            });

            volumeSeries.current.priceScale().applyOptions({
                position: "right", // Ensure the scale is on the right side
                scaleMargins: { top: 0.8, bottom: 0 },
            });

            candlestickSeries.current.priceScale().applyOptions({
                position: "left", // Keep the candlestick scale on the left side
