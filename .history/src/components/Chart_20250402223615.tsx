    ...
    return (
        ...
        <div className={`${styles.container} ${darkMode ? "bg-dark" : "bg-light"}`}>
            <div className={styles.leftPanelWrapper}>
                <div className={`${styles.topBar} ${darkMode ? "bg-dark-header" : "bg-light-header"}`}>
                    <strong className={styles.cryptoName}>{crtytoName}</strong>
                    <div className={styles.controls}>
                        <button
                            onClick={toggleDarkMode}
                            className={`${styles.darkModeButton} ${darkMode ? "btn-dark" : "btn-light"}`}
                            aria-label={darkMode ? "Switch to light mode" : "Switch to dark mode"}
                        >
                            {darkMode ? <FaMoon size={18} /> : <FaSun size={18} />}
                        </button>
                        <select
                            value={time}
                            onChange={(e) => setTime(e.target.value)}
                            className={`${styles.selectTime} ${darkMode ? "select-dark" : "select-light"}`}
                            aria-label="Select time frame"
                        >
                            {times.map((item, index) => (
                                <option key={index} value={item.value} className={darkMode ? "bg-gray-800" : "bg-white"}>
                                    {item.label}
                                </option>
                            ))}
                        </select>
                ...
                <div className={`${styles.chartWrapper} ${darkMode ? "border-gray-800" : "border-gray-200"}`} ref={chartContainer}></div>
            </div>

            <div className={`${styles.rightPanel} ${darkMode ? "bg-dark-sidebar" : "bg-light-sidebar"}`}>
                <div className={`${styles.cryptoListHeader} ${darkMode ? "text-white" : "text-black"}`}>Crypto List</div>
                <ul className={`${styles.cryptoList}`}>
                    {cryptoCoins.map((item, index) => (
                        ...
                        >
                            <img className={styles.cryptoIcon} src={item.cryptoImage || "/placeholder.svg"} alt={item.cryptoName} />
                            <span className={styles.cryptoNameText}>{item.cryptoName}</span>
                        </li>
                    ))}
                </ul>
                <div className={`${styles.priceButtonWrapper} ${darkMode ? "border-gray-700" : ""}`}>
                    <button onClick={handlePrice} className={styles.getPriceButton}>
                        Get Current Price
                    </button>

                    {show && (
                        <div className={`${styles.priceInfoWrapper} ${darkMode ? "card-dark" : "card-light"}`}>
                            <div className={styles.priceItem}>
                                <span className={styles.priceLabel}>Current Price:</span>
                                <span className={styles.priceValue}>${prices.current?.toLocaleString()}</span>
                            </div>
                            <div className={styles.priceItem}>
                                <span className={styles.priceLabel}>One Minute Ago:</span>
                                <span className={styles.priceValue}>${prices.oneMinuteAgo?.toLocaleString()}</span>
                            </div>
                        </div>
                    )}
                </div>
