...
export const GetCandles = async (currentTimeFrame: string, currentCoin: string): Promise<ICandleStick[]> => {
    try {
        const response = await axios.get<[number, string, string, string, string, string, number, string, number, string, string, string][]>(
            `https://api.binance.com/api/v3/klines?symbol=${currentCoin}&interval=${currentTimeFrame}&limit=5000`
        );
...
export const GetPrice1MinuteAgo = async (currentCoin: string) => {
    try {
        if (!candles || candles.length < 2) {
            console.error("Not enough candle data for", currentCoin);
            return 0; // Giá trị mặc định tránh lỗi
        }

        const priceOneMinuteAgo = candles[candles.length - 2]?.close;

        // Kiểm tra nếu priceOneMinuteAgo không hợp lệ
        if (typeof priceOneMinuteAgo !== "number" || isNaN(priceOneMinuteAgo)) {
            console.error("Invalid price data for", currentCoin, ":", priceOneMinuteAgo);
            return 0; // Giá trị mặc định tránh lỗi
        }

        return priceOneMinuteAgo;
    } catch (error) {
        console.error("Error fetching price 1 minute ago:", error);
        return null;
    }
};
