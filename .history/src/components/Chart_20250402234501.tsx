...
export default function Chart() {
    ...
    useEffect(() => {
        ...
        if (!chartRef.current) {
            ...
            // Candlestick series with its own scale
            candlestickSeries.current = chartRef.current.addSeries(CandlestickSeries, {
                ...
                priceScaleId: "candlestick", // Separate scale for candlestick
            });

            chartRef.current.priceScale("candlestick").applyOptions({
                position: "right", // Candlestick scale on the right
                scaleMargins: { top: 0.2, bottom: 0.7 }, // Adjust margins to separate from volume
                borderVisible: true,
                borderColor: darkMode ? "#1e222d" : "#e0e0e0",
            });

            // Volume series with its own scale
            ...
            chartRef.current.priceScale("volume").applyOptions({
                position: "right", // Volume scale on the right
                scaleMargins: { top: 0.8, bottom: 0 }, // Adjust margins to separate from candlestick
                borderVisible: true,
                borderColor: darkMode ? "#1e222d" : "#e0e0e0",
            });
